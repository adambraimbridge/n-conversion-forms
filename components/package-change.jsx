...
export function PackageChange({
	...
	const nameMap = {
		annual: {
			...
			price: (term) => (
				<React.Fragment>
					Single{' '}
					<span className="ncf__package-change__price">{term.price}</span>{' '}
					payment*
				</React.Fragment>
			),
			weeklyPrice: (term) => (
				<div className="ncf__package-change__weekly-price">
					Just{' '}
					<span className="ncf__package-change__price">{term.weeklyPrice}</span>{' '}
					per week
				</div>
			),
		},
		quarterly: {
			...
			price: (term) => (
				<React.Fragment>
					<span className="ncf__package-change__price">{term.price}</span> per
					quarter
				</React.Fragment>
			),
			weeklyPrice: () => {},
		...
		trial: {
			...
			price: (term) => (
				<React.Fragment>
					4 weeks for{' '}
					<span className="ncf__package-change__trial-price">
						{term.trialPrice}
					</span>
				</React.Fragment>
			),
	...
	const createDiscount = () => {
		return (
			terms.find((term) => term.name === 'annual' && !term.discount) && (
				<div className="ncf__package-change__annual-copy">
					* Save up to 25% when you pay annually. Discount varies across member
					states and across our subscription packages.
				</div>
			)
		);
	};
	...
	const createTerm = (term) => {
		...
		const discount = term.discount && (
			<span className="ncf__package-change__discount">
				Save {term.discount}
			</span>
		);
		return (
			<div key={term.name} className={className}>
				<span className="ncf__package-change__title">
					{nameMap[term.name].title}
				</span>
				{discount}
